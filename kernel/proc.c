...
found:
	...
	*(uint*)sp = (uint)trapret;
	...
	//PODESAVAMO SVE SHARE SA 0 SIZE KOD NOVOG PROC
	int i;
	for (i = 0; i < SHNUM; i++)
	{
		p->shares[i].size = 0;
...
fork(void)
{
	...
	*np->tf = *curproc->tf;
	...
	// NASLEDJIVANJE SHARE STRUKTURA RODITELJA
	int j;
	for (j = 0; j < SHNUM; j++)
	{
		np->shares[j] = curproc->shares[j];
	}
	// CUVANJE CR3 RODITELJA U DETETU
	if (j > 0)
		np->PARENTpgdir = curproc->pgdir;
	else
		np->PARENTpgdir = np->pgdir;
